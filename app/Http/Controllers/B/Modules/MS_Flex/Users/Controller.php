...
return  redirect()->action('\BECM\MS_Flex\Users\Controller@login')->with('errors',\collect(['Username/Password invalid.',"Please Enter Valid Username/Password."]));
    ...
    \MS\Plugin\SMS::send($number,"For User:  ".$R->input('UserName')."\nYou are logged in from MS-Flex");
...
public function logout(Request $R){
    ...
    $number=$row->pluck('MobileNumber')->first();
    $username=$row->pluck('Username')->first();

    \MS\Plugin\SMS::send($number,"For User:  ".$username."\nYou are logged out from MS-Flex");
